...
export default function OpportunitiesPage() {
  ...
  function generateBadge() {
    try {
      ...
      // Flatten the per-skill occupations map into a deduped list of
      // { name, matching_percentage, description, skills[] } for the badge.
      const occByName: Record<string, { name: string; matching_percentage?: number; description?: string; skills: Set<string> }> = {};
      Object.entries(occupationsBySkill).forEach(([skill, occs]) => {
        Object.entries(occs || {}).forEach(([name, info]) => {
          if (!occByName[name]) {
            occByName[name] = {
              name,
              matching_percentage: info?.matching_percentage,
              description: info?.description,
              skills: new Set<string>(),
            };
          }
          occByName[name].skills.add(skill);
        });
      });
      const occupationsList = Object.values(occByName)
        .map((o) => ({
          name: o.name,
          matching_percentage: o.matching_percentage,
  ...
  useEffect(() => {
    try {
      ...
      const cached = storage.getOccupationsBySkill();
      if (cached && Object.keys(cached).length > 0) {
        setOccupationsBySkill(cached as Record<string, OccupationsResponse>);
      }
    } catch (e) {
    ...
    const timer = setTimeout(() => {
      async function fetchPerSkill() {
        ...
        if (!cancelled) {
          ...
          setOccupationsBySkill(map);
          try {
            storage.mergeOccupationsBySkill(map as any);
          } catch (e) {
            console.warn('Failed to persist occupationsBySkill', e);
          }
        }
      }
